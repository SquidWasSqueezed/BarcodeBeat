...
void setup(){  //Setup mit Import des Barcodes und Ausführung der Unterprogramme.
  ...
  Zeile = loadImage("Zeile unten.PNG");
  imageMode(CENTER);
  image(barcode,450,200,900,400); 
  image(Zeile, 100, 450);
}
...
void barcodeAnzeigen() {  //zeichnet den Barcode auf den Bildschirm.
  ...
  output = createWriter("RecentScan" + h + "_" + m + ".txt");
...
println("Auslese gestartet!");
  int hellste = -100000000;
  int dunkelste = 0;
  for(int i = 0; i < 1001; i++){
    for(int i2 = 0; i2 < 1901 ; i2++){
      if(get(i2,i) > hellste){
        hellste = get(i2,i);
      }
      if(get(i2,i) < dunkelste){
        dunkelste = get(i2,i);
      }
    }
  }
  mittelwert = hellste + dunkelste  / 2;

  int counter = 0;
  int counter2 = 0;
   for(int i = 1000; (i >= 0) && (ende == 0); i--){
     if(get(i,40) < mittelwert){
       ende = i+10;
     }
  }
  int[] helpy = new int[5];
  for(int i3 = 0;i3<1901 ; i3++){
    helpy[0] = get(i3,40);
    helpy[1] = get(i3,80);
    helpy[2] = get(i3,120);
    helpy[3] = get(i3,160);
  ...
  int achtung = 0;
  for(int i = 0;achtung == 0 && i<1901;i++) {  //einstellen des Programms auf im Barcode vorgegeben Balkengröße 1 (mit c1).
    if(get(i,40)<mittelwert) {
      help++;
    }else{
      if(help!=0){
...
void pruefziffer() {   //Errechnen und Prüfen der Prüfziffer
  ...
  for(int i = 0;i<13;i++) {
    ...
    output = createWriter(zahlen[i] + ".json");
